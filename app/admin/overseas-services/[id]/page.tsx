...
interface FormData {
  ...
  nameVi: string;
  icon: string;
  gallery: string;        // JSON array of image URLs (max 3)
  description: string;
  ...
  contentTh: string;
  contentVi: string;
  advantages: string;     // JSON array of {icon, text}
  advantagesEn: string;
  advantagesTh: string;
  advantagesVi: string;
  sortOrder: number;
...
export default function OverseasServiceEditPage() {
  ...
  // Gallery images state for UI
  const [galleryImages, setGalleryImages] = useState<string[]>([]);
  // Advantages editing state
  ...
  // Sync galleryImages when form.gallery changes
  useEffect(() => {
    setGalleryImages(parseGallery(form.gallery));
  }, [form.gallery]);

  // Sync advantageItems when the active language's advantages field changes
  useEffect(() => {
  ...
  const loadService = async (serviceId: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/overseas-services/${serviceId}`);
      ...
      setForm({
        ...
        advantagesEn: data.advantagesEn || '[]',
        advantagesTh: data.advantagesTh || '[]',
        advantagesVi: data.advantagesVi || '[]',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = id 
        ? `/api/overseas-services/${id}`
        : '/api/overseas-services';
  ...
  /** Update a single gallery image at index */
...
// (moved above - already defined before useEffects)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                disabled={!!id}
              />
              <p className="text-xs text-slate-400 mt-1">唯一标识，用于URL路径，创建后不可修改</p>
            </div>
