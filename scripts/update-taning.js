...
async function main() {
  ...
  console.log('Current brandStory:', brand.brandStory?.substring(0, 80));
  console.log('Current brandFeatures:', brand.brandFeatures?.substring(0, 80));
  
  const updates = {
    // 品牌故事
    brandStory: `2017年，挞柠（TANING）在广州江南新地开设了第一家手打柠檬茶专营店，将柠檬茶从传统茶饮店中独立出来，开创了柠檬茶专门店的先河。

"挞"（tà），是广东方言中"手打"的意思——每一杯挞柠柠檬茶，都经过人工捶打，将柠檬的香气与茶汤完美融合。
...
• 选用4种不同品种柠檬，搭配6种精选茶叶，为顾客提供个性化柠檬茶饮
• 率先将凤凰单枞应用于茶饮行业，鸭屎香柠檬茶成为标志性爆款

【供应链优势】
• 潮州凤凰山自有40亩茶园，国内少数拥有自有茶园的茶饮品牌
• 自有果园与原物料工厂，全产业链布局，确保原料品质与成本优势
...
• 全球9国68城1000+门店（截至2025年）`,
  ...
  console.log('\n✅ 更新成功！');
  console.log('品牌:', updated.name);
  console.log('品牌故事长度:', updated.brandStory?.length);
  console.log('品牌特色长度:', updated.brandFeatures?.length);
  console.log('流程步骤数:', JSON.parse(updated.process || '[]').length);
  console.log('支持项目数:', JSON.parse(updated.support || '[]').length);
}
